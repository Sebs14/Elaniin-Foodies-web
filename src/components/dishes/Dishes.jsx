...
const Dishes = () => {
  ...
  const fetchText = async () => {
    const text = await getText('dish');
    setTexts(text.attributes);
    console.log(text);
  }
      ...
      <div className="flex items-center justify-center space-x-4 pt-10">
        {currentPage != undefined ? (
          page > 1 ? (
            <ButtonPrev
              classes={
                "relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-black transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-yellow-300 group"
              }
              click={handlePagesPrevious}
              text={texts?.buttonPrev}
            />
          ) : (
            ""
          )
